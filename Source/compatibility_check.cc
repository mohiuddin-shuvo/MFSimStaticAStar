...
void CompatChk::PreRouteChk(Router *r, DmfbArch *arch, bool runAsEntireFlow)
{
	...
	else if (r->getType() == A_STAR_TYPE_1_R || r->getType() == A_STAR_TYPE_2_R || r->getType() == A_STAR_TYPE_3_R ||
			 r->getType() == PR_A_STAR_TYPE_1S_R || r->getType() == PR_A_STAR_TYPE_2S_R || r->getType() == PR_A_STAR_TYPE_3S_R ||
			 r->getType() == PR_A_STAR_TYPE_1L_R || r->getType() == PR_A_STAR_TYPE_2L_R || r->getType() == PR_A_STAR_TYPE_3L_R ||
			 r->getType() == PR_A_STAR_TYPE_1R_R || r->getType() == PR_A_STAR_TYPE_2R_R || r->getType() == PR_A_STAR_TYPE_3R_R ||
			 r->getType() == PR_A_STAR_TYPE_1X_R || r->getType() == PR_A_STAR_TYPE_2X_R || r->getType() == PR_A_STAR_TYPE_3X_R ||
			 r->getType() == PR_A_STAR_TYPE_1Y_R || r->getType() == PR_A_STAR_TYPE_2Y_R || r->getType() == PR_A_STAR_TYPE_3Y_R ||
			 r->getType() == PR_A_STAR_TYPE_1B_R || r->getType() == PR_A_STAR_TYPE_2B_R || r->getType() == PR_A_STAR_TYPE_3B_R )
	{
		// The A* routers do not require an additional compaction step
		claim(r->getCompactionType() == INHERENT_COMP, "The A* and prioritized A* routers must be paired with the inherent compaction type.");
	}
	else
	{
