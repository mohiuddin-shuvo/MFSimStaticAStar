 ...
 * Source: grissom_fppc_path_scheduler.cc										*
 * Original Code Author(s): Dan Grissom											*
 * Original Completion/Release Date: March 6, 2014								*
 *																				*
 * Details: N/A																	*
 *																				*
 * Revision History:															*
...
// Constructor
///////////////////////////////////////////////////////////////////////////////////
GrissomFppcPathScheduler::GrissomFppcPathScheduler()
{
...
/////////////////////////////////////////////////////////////////
ResHist * GrissomFppcPathScheduler::createNewResHist(unsigned long long ts)
{
...
// map.
/////////////////////////////////////////////////////////////////
ResHist * GrissomFppcPathScheduler::getResHistAtTS(unsigned long long ts)
{
...
// specified TS
/////////////////////////////////////////////////////////////////
int GrissomFppcPathScheduler::getNumNewIndefStorDrops(unsigned long long ts)
{
...
/////////////////////////////////////////////////////////////////
int GrissomFppcPathScheduler::getNumIndefStorDropsBefore(unsigned long long ts)
{
	int numDrops = 0;
...
list<AssayNode *> *GrissomFppcPathScheduler::getIndefStorDropsBefore(unsigned long long ts)
{
...
/////////////////////////////////////////////////////////////////
bool GrissomFppcPathScheduler::resTypeExists(ResourceType rt)
{
...
/////////////////////////////////////////////////////////////////
IoResource * GrissomFppcPathScheduler::getReadyDispenseWell(string fluidName, unsigned long long schedTS, unsigned long long dagStartTS)
{
...
/////////////////////////////////////////////////////////////////
/*int GrissomFppcPathScheduler::getNumFreeModules(unsigned long long ts, int runningStorageDrops)
{
...
// time-step
/////////////////////////////////////////////////////////////////
bool GrissomFppcPathScheduler::resTypeIsAvailAtTS(unsigned long long ts, OperationType ot, ResourceType rt, int runningStorageDrops)
{
	if (rt == BASIC_RES)
	{
		if (getResHistAtTS(ts)->availRes[BASIC_RES] > 0) // DTG
			return true;

		/*if (getResHistAtTS(ts)->availRes[SSD_RES] - runningStorageDrops < 0)
		{
			if (getResHistAtTS(ts)->availRes[BASIC_RES] - (runningStorageDrops - getResHistAtTS(ts)->availRes[SSD_RES]) > 0)
				return true;
		}
		else if (getResHistAtTS(ts)->availRes[BASIC_RES] > 0)
			return true;*/
	}
	else if (rt == SSD_RES)
	{
		if (ot == SPLIT)
		{
			// Split operations need 2 SSD Moduls for storage of second droplet
			if (getResHistAtTS(ts)->availRes[SSD_RES] - runningStorageDrops >= 2)
				return true;
		}
		else if (getResHistAtTS(ts)->availRes[SSD_RES] - runningStorageDrops >= 1)
			return true;

	}
	else
		claim(false, "FPPC Path Scheduler only expects Basic and SSD (split-store-merge) resources.");
...
/////////////////////////////////////////////////////////////////
bool GrissomFppcPathScheduler::canStoreDropsAtTS(unsigned long long ts, int numDrops)
{
	if (getResHistAtTS(ts)->availRes[SSD_RES] - numDrops > 0) // DTG
	//if (getResHistAtTS(ts)->availRes[SSD_RES] + getResHistAtTS(ts)->availRes[BASIC_RES] - numDrops > 0) // DTG
		return true;
	else
		return false;
...
/////////////////////////////////////////////////////////////////
void GrissomFppcPathScheduler::setActiveAtTS(IoResource *input, unsigned long long ts)
{
...
/////////////////////////////////////////////////////////////////
void GrissomFppcPathScheduler::setInactiveAtTS(IoResource *input, unsigned long long ts)
{
	map<unsigned long long, bool>::iterator it;
	for (unsigned i = ts - input->durationInTS; i < ts; i++)
	{
...
/////////////////////////////////////////////////////////////////
void GrissomFppcPathScheduler::setPotentialRes(vector<ResourceType> *potRes, OperationType ot)
{
	if (ot == MIX || ot == DILUTE)
	{
	...
	else if (ot == SPLIT || ot == DETECT || ot == HEAT)
	{
		if (resTypeExists(SSD_RES))
			potRes->push_back(SSD_RES);
	}
}
...
/////////////////////////////////////////////////////////////////
unsigned long long GrissomFppcPathScheduler::schedule(DmfbArch *arch, DAG *dag)
{
	//long statPathNo = 0;
	...
	while (moreNodesToSchedule())
	{
		...
		while (continuePath)
		{
			...
			for (int i = 0; i < potResTypes.size(); i++)
			{
				...
				while (!gapIsFound)
				{
					...
					if (resTypeIsAvailAtTS(schedTS, n->type, rt, runDropsInStorage))
					{
						consecRun++;
		...
		{ 	// Submit path
			...
			for (unsigned pi = 0; pi < path.size(); pi++)
			{
				...
				// If SPLIT node, convert to two stores by making the Split "instantaneous" so we can reserve two modules for it
				if (pathNode->type == SPLIT)
					pathNode->endTimeStep = pathNode->startTimeStep;
				...
				{
					if (pathNode->type == SPLIT)
						getResHistAtTS(dur)->availRes[pathNode->boundedResType]--; // Split was reserving 2 SSD modules
	...
	for (; rhIt != availResAtTS->end(); rhIt++)
	{
		...
		if (rh->timeStep >= minInTS && rh->timeStep < maxOutTS)
		{
			...
			while (dis > 0)
			{
				...
				if (rh->availRes[SSD_RES] > 0)
					rh->availRes[SSD_RES]--;
				//else if (rh->availRes[BASIC_RES] > 0)
				//	rh->availRes[BASIC_RES]--;
				else
				{
					stringstream msg;
					msg << "ERROR: Insufficient resources for scheduling. Attempting to store a droplet at TS " << rh->timeStep;
					msg << ", but no more storage modules are available." << endl;
					claim(false, &msg);
				}
			}
			//rh->availRes[SSD_RES] = rh->availRes[SSD_RES] - rh->dropsInStorage; // DTG
		}
	}
